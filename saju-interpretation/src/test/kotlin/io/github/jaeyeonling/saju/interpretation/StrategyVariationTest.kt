...
class StrategyVariationTest : StringSpec({
    ...
    "양포태 — 음간은 같은 오행 양간의 장생지에서 순행한다 (핀값)" {
        ...
        withClue("기@인=장생(무와 동일·화토동법)") { YangPotaeStrategy.stageOf(Cheongan.GI, Jiji.IN) shouldBe SibiUnseong.JANGSAENG }
    ...
    "투출 격국 — 비겁 투출은 격으로 삼지 않는다 (양인격 버그 회귀)" {
        ...
        withClue("비겁(을) 투출 무시 → 본기 무=편재격") { TuchulGyeokgukStrategy.DEFAULT.classify(chart).type shouldBe GyeokgukType.PYEONJAE }
    }

    "억부 가중치를 바꾸면 지원율이 달라진다" {
        val chart = sampleChart()
        val default = BueokSinStrengthStrategy.DEFAULT.evaluate(chart).supportRatio
        val noHidden = BueokSinStrengthStrategy(
    ...
    "withHiddenStems 는 다른 전략 교체와 합성된다 (덮어쓰지 않음)" {
        ...
        val composed = InterpretationContext.DEFAULT
...
private fun chartWithMonth(monthJi: Jiji): SajuChart = SajuChart(
...
private fun sampleChart(): SajuChart = SajuChart(
    year = Pillar(PillarPosition.YEAR, GanZhi.fromIndex(0)),
