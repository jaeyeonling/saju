...
private const val ONE_ARCSEC_DEG = 1.0 / 3600.0

class SunPositionTest : StringSpec({

    "Meeus Example 25_b — 1992-10-13 0h TD 태양 겉보기 황경" {
        ...
        longitudeDeg shouldBe (expectedDeg plusOrMinus ONE_ARCSEC_DEG)
    }

    "춘분점 부근 — 2000년 춘분 태양 황경은 0도 근처" {
        ...
        withClue("춘분 황경이 0°에서 너무 멀다: $longitudeDeg°") { (distanceToZero < 0.05).shouldBeTrue() }
    }

    "황경은 시간에 따라 단조 증가한다 (하루 약 0_985도)" {
        ...
        withClue("일주 황경 변화가 비정상: $dailyMotion°/일") { (dailyMotion in 0.95..1.02).shouldBeTrue() }
    }
})
