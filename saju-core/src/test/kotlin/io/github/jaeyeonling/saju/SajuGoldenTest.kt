...
class SajuGoldenTest {
    ...
    @Test
    fun `대표 출생 시각의 4기둥이 tyme4j 와 일치`() {
        for (s in SAMPLES) {
    ...
    @Test
    fun `1900~2050 매년 표본 4기둥이 tyme4j 와 일치`() {
    ...
    private fun assertChartMatches(year: Int, month: Int, day: Int, hour: Int, minute: Int) {
        ...
        assertEquals(tyme.year.index, mine.year.ganZhi.index, "연주 @ $tag")
        assertEquals(tyme.month.index, mine.month.ganZhi.index, "월주 @ $tag")
        assertEquals(tyme.day.index, mine.day.ganZhi.index, "일주 @ $tag")
        assertEquals(tyme.sixtyCycle.index, mine.hour.ganZhi.index, "시주 @ $tag")
    }
