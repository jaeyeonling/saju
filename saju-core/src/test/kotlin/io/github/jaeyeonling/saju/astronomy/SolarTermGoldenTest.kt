 ...
 *
 * 골든 벡터는 외부 만세력 라이브러리 tyme4j 출력을 베이징(UTC+8) 기준 JD 로 동결한 것이다
 * (CSV 컬럼 `tymeIndex` 가 그 출처를 가리킨다). tyme4j 도 VSOP87 계열 황경을 쓰므로 자체 엔진과
 * 수학 계보를 일부 공유한다 — 이 회귀는 '두 구현의 분 단위 합치'를 박제한 것이고, 정설 천문학에 대한
 * 절대 앵커는 단위 테스트(SunPositionTest 의 Meeus Example 25.b, ~1″)가 맡는다.
 *
