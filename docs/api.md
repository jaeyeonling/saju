...
| `KoreanSaju.daeun(y, mo, d, h, mi, gender, longitudeDeg=서울, count=8)` | 생년월일시 + **성별** → `List<Daeun>` | 10년 단위 대운. **성별(`Gender`)은 순행·역행 방향 전용 — 원국 무관.** `isMale: Boolean` 오버로드도 유지 |
...
| `Saju.daeun(utJd, monthPillar, yearStemEumyang, gender, count=8)` | `List<Daeun>` (성별=방향 전용; `isMale: Boolean` 오버로드도 유지) |
...
Daeun     ─ startAge : Int · ganji : Ganji            (직렬화: DaeunDto / 성별 포함 DaeunSeriesDto — daeun.toDaeunJson(gender))
...
| 필드 | 타입 | 의미 | 접근 예 |
|------|------|------|---------|
| `strength` | `SinStrength` | 신강신약 | `.verdict` · `.supportRatio` · `.basis` · `.groupScores`(십성 5묶음 세력) |
| `yongsin` | `YongsinResult` | 용신(균형 오행) | `.yongsin`(Ohaeng) · `.method` · `.basis`(억부 분기 근거) |
| `gyeokguk` | `GyeokgukResult` | 격국(짜임새) | `.type` · `.basis` |
| `gongmang` | `Pair<Jiji, Jiji>` | 공망(빈 지지 2개) | `.first` · `.second` |
| `hapChung` | `List<HapChungRelation>` | 합·충 관계(천간합·천간충·육합·육충·육해·삼합·방합 7종 sealed) | `filterIsInstance<CheonganChung>()` · `CheonganHap.transformsTo`(일간 낀 합은 `null`=합화 보류) |
...
| `Gender` 성별 | 남(MALE) · 여(FEMALE) | `.isMale`(대운 방향 판정 브릿지) · `.koreanName` · `Gender.fromCode("M"/"F"/"남"/"여")` — 그 외 입력은 `IllegalArgumentException`(대운 방향이 뒤집히므로 조용히 흡수하지 않음). 원국 무관·대운 방향 전용 |
...
> **억부 파라미터 튜닝** — 전략 교체 없이 `EokbuSinStrengthStrategy(EokbuWeights.DEFAULT.copy(...))` 로 미세 조정한다.
> 월령·지장간(정/중/여기) 가중에 더해 **통근 가중**(`rootStrong` 장생·건록·제왕 / `rootWeak` 묘 / `rootMid` 나머지)을 연다.
> 통근 3필드는 **학파 의존이라 기본 1.0(중립)** — 건드리지 않으면 기존 verdict·용신이 그대로 보존된다.
