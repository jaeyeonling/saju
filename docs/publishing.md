...
> 현재 최신 릴리스: **v0.1.0** (2026-07-02, Maven Central 게시 완료).
> 아래는 **다음** 릴리스 절차다 — 예시의 `v0.1.0` 은 이미 게시됐으므로 다음 버전(예: `v0.1.1`)으로 읽는다.

1. `gradle.properties` 의 `VERSION_NAME` 을 올린다 (예: `0.1.1`).
...
`saju.publish` convention plugin 이 적용된 5개 라이브러리 모듈이 [Maven Central 에 게시](https://central.sonatype.com/artifact/io.github.jaeyeonling/saju-core)된다(그룹 `io.github.jaeyeonling`):
`saju-core`, `saju-korea`, `saju-interpretation`, `saju-group`, `saju-serialization`.
`saju-cli` 는 데모 앱이라 배포 대상이 아니다. 소비자용 의존성 사용법은 [README 설치 섹션](../README.md#설치) 참조.
