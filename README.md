...
용신     : 화(火) (억부)
         └ 중화(47%) · 관성과다(관성 4.0) → 인성 화(火) 설기·생조   ← 억부 분기 근거(무엇이 과한가)
격국     : 편관격
         └ 월지(묘) 본기 을 → 편관
공망     : 신유
...
합충     : (있으면) 천간합(갑-기→토) · 천간충(갑-경) · 육합/육충/육해 · 삼합/방합(인-묘-진→목) 상세  ← 일간 낀 천간합은 합화(化) 보류
...
사주(四柱)는 전통 명리학, 즉 동양 점술의 한 갈래다. 이 라이브러리는 그 **규칙과 계산을 결정론적으로 구현한 기술 도구**일 뿐, 사주가 미래·성격·운명을 실제로 예측한다는 어떤 주장도 하지 않는다.

- 산출물(사주판·해석·대운 등)은 **문화적·교육적·오락적 참고 목적**이다.
- 의료·법률·재무·심리·진로 등 **전문적 판단이나 중대한 의사결정의 근거로 삼지 말 것.** 그런 결정은 자격 있는 전문가와 상의하라.
- 천문·도출(절기·4기둥)은 골든 벡터와 Meeus 원전으로 검증되지만, **해석 영역(신강신약·용신·격국 등)은 유파에 따라 달라지는 관습적 규칙**이며 객관적 정답이 존재하지 않는다(위 "해석의 정확도 한계" 주석 참조).
- [MIT 라이선스](LICENSE)의 무보증(AS IS) 조항이 그대로 적용된다 — 사용으로 인한 결과에 대해 저자는 책임지지 않는다.
